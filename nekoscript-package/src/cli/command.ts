...
export class NekoCommand {
  ...
  private parser: NekoParser;
  private interpreter: NekoInterpreter;
   ...
   * @returns Un message de résultat
   */
...
╚════════════════════════════════════════════════╝
   ...
   * @param packageName Nom du package à installer
   */
...
Note: Le fichier n'a pas pu être écrit sur disque.`);
   ...
   * @param description Description du package
   */
...
🌐 Les autres peuvent l'installer avec: neko-script librairie ${packageName.replace('.neko', '')}`);
   ...
   * @param fileName Nom du fichier à exécuter
   */
